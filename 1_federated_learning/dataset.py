...
class PlantDataset(torch.utils.data.Dataset):
	def __init__(self, set_dir, client_dir, transform=None, target_transform=None):
		...
		# if client_dir: client dataset used for training
		if self.client_dir:
			self.df = self.df[self.df['Rfid'] == self.client_dir]

			if self.set_dir == 'train':
				self.set_df = self.df[:100]
			elif self.set_dir == 'valid':
				self.set_df = self.df[100:]

		# if client_dir not specified: full dataset used for validation
		else:
